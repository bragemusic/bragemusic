...
export function LocalPlayer() {
    ...
    const [ctx, setCtx] = useState<types.PlayContext>(() => {
        const ctx = new types.PlayContext();
        ctx.type = "";
        ctx.ref_id = "";
        ctx.tracks = [];
        ctx.track_order = [];
    ...
    useEffect(() => {
        ...
        setCtx((prev) => {
            ...
            next.type = prev.type;
            next.ref_id = prev.ref_id;
            next.tracks = prev.tracks;
            next.track_order = trackOrder;
    ...
    const updatePb = (
        update: (prev: types.PlaybackState) => types.PlaybackState,
        progress?: number,
    ) => {
        setPb((prev) => ({
            ...update(prev),
    ...
    useEffect(() => {
        const unsubscribePlayerLocalStartContext = api.eventSubscribe(
            Event.PlayerLocalStartContext,
            (lctx: LocalPlayerContext) => {
                const { trackOrder, trackIndex } = shuffleTrackOrder(
                    lctx.tracks,
        ...
        const unsubscribePlayerLocalNextTrack = api.eventSubscribe(
            Event.PlayerLocalNextTrack,
            () => {
                nextTrack();
            },
        );

        const unsubscribePlayerLocalPreviousTrack = api.eventSubscribe(
            Event.PlayerLocalPreviousTrack,
            () => {
                previousTrack();
            },
        );

        const unsubscribePlayerLocalRepeat = api.eventSubscribe(
            Event.PlayerLocalRepeat,
            (repeat: string) => {
                updatePb((prev) => ({
                    ...prev,
                    repeat: repeat,
        ...
        const unsubscribePlayerLocalShuffle = api.eventSubscribe(
            Event.PlayerLocalShuffle,
            (shuffle: boolean) => {
                updatePb((prev) => ({
                    ...prev,
                    shuffle: shuffle,
    ...
    const startCurrentTrack = () => {
        ...
        audioRef.current!.src =
            "/api/mediafiles/" + currentTrack.media_file.id + "/file";
        audioRef.current?.play();
    };
